...
def insert_playlist(uid):
    data = cloudmusic_new.get_user_fans(uid)
    if data['code'] == 200:
        json.dump(data, open('../data/fan/{}.txt'.format(uid), 'w'), indent=4, ensure_ascii=False)
...
def main():
    ...
    have_uids = set([os.path.splitext(f_name)[1] for f_name in os.listdir('../data/fan/')])
...
if __name__=='__main__':
    try:
        ...
        send_email.alert('获取用户粉丝详情：' + str(e))

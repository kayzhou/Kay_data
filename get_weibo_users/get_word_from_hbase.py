...
def get_weibo_user_month(list_id, date):
    ...
    with open('data/word/word.txt' % date, 'w') as f:
        for k, v in rst.items():
            f.write(k + '\t' + v + '\n')


def get_word_month(list_id, date):
    rst = db_hbase.scan_word(list_id, 'search_keytweets' + date[:-2], date)
